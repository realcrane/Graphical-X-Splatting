...
import shutil
from utils.colmap_utils import COLMAPDatabase
...
def write_colmap(path, cameras, offset=0):
    ...
    for cam in cameras:
        ...
        camera_id = db.add_camera(1, int(w), int(h), params)
        cameraline = f"{id} PINHOLE {int(w)} {int(h)} {fx} {fy} {cx} {cy}\n"
...
# --- Pose / intrinsic conversion helpers (used only by preprocessing scripts) ---
